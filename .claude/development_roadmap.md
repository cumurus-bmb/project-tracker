...
### 4.1 作業履歴表示
- [x] `Components/WorkHistory.tsx` の作成
  - [x] 日付ごとのグルーピング（ユーザーのタイムゾーン対応）
  - [x] カテゴリ表示（色付きドット）
  - [x] 作業時間の表示（開始〜終了時刻 + 合計時間）
  - [x] 編集・削除機能（Inertia router 使用）
  - [x] メモの表示
...
- [x] `ReportController@index` の作成
- [x] `Pages/Reports/Index.tsx` の作成
  - [x] 日次/週次/月次/年次タブ
  - [x] 期間セレクター（今日/昨日/カスタム・今週/先週・今月/先月・今年/去年）
  - [x] サマリーカード（合計時間・前期間比・1日平均）
  - [x] カテゴリ別内訳
...
- [x] Recharts のインストール
  ...
  ```
- [x] グラフコンポーネントの作成
  - [x] 棒グラフ（時間帯別/曜日別/日別/月別）
  - [x] 円グラフ（カテゴリ別時間配分 + 凡例）
  - [x] ヒートマップ（月次：カレンダー形式 / 年次：GitHub 形式）
  - [x] カテゴリ別内訳（プログレスバー付き）
